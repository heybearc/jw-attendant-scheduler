...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'GET') {
      // Anyone with event access can view positions
      const hasAccess = await checkEventAccess(user.id, eventId, 'VIEWER')
      if (!hasAccess) {
        return res.status(403).json({ success: false, error: 'You do not have permission to view this event' })
      }
      return await handleGetPosition(req, res, position)
    } else if (req.method === 'PUT' || req.method === 'DELETE') {
      // Check if user can manage this specific position
      const canManage = await canManagePosition(user.id, eventId, positionId)
      if (!canManage) {
        return res.status(403).json({ success: false, error: 'You do not have permission to manage this position' })
      }
