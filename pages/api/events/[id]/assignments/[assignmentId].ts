...
import { canEditAssignment } from '../../../../../src/lib/eventAccess'
  ...
  try {
    ...
    switch (req.method) {
      case 'DELETE':
        ...
        const canEdit = await canEditAssignment(user.id, eventId, assignmentId)
        if (!canEdit) {
          return res.status(403).json({ error: 'You do not have permission to remove this assignment' })
        }
