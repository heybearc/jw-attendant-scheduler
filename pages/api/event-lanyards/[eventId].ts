  ...
  try {
    ...
    switch (req.method) {
      case 'GET':
        ...
        // Fetch current attendant info for checked-out lanyards
        const lanyardsWithAttendantInfo = await Promise.all(
          lanyards.map(async (lanyard) => {
            if (lanyard.isCheckedOut && (lanyard as any).attendantId) {
              try {
                ...
                if (attendant) {
                  // Update notes with current phone number
                  return {
                    ...lanyard,
                    notes: attendant.phone || lanyard.notes
                  }
                }
              } catch (error) {
                console.error(`Error fetching attendant for lanyard ${lanyard.id}:`, error)
              }
            }
            return lanyard
          })
        ...
        const sortedLanyards = lanyardsWithAttendantInfo.sort((a, b) => {
