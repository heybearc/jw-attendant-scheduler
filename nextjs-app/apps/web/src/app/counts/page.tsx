...
import { CountService, CountSessionWithEvent } from '../../../libs/count-tracking/src/countservice';

export default function CountsPage() {
  const [countSessions, setCountSessions] = useState<CountSessionWithEvent[]>([]);
          ...
          countSessions.map((session) => (
            <div key={session.id} className="bg-white p-6 rounded-lg shadow border hover:shadow-lg transition-shadow">
              ...
              <div className="border-t pt-3 mt-4 text-sm text-gray-500">
                ...
                <p>Event Date: {new Date(session.event.date).toLocaleDateString()}</p>
              </div>
            </div>
          ))
        )}
